...
std::vector<unsigned int> reconstruct_a_star_path(const std::unordered_map<unsigned int, unsigned int>& came_from, unsigned int current_position);

template <class T>
void compute_astar(Graph<T>& graph_, unsigned int start_position, unsigned int goal_position) {
    ...
    for (const auto& vertex_entry : graph_.get_vertices())
    {
        ...
        f_score[vertex_position] = (vertex_position == start_position) ? graph_.get_heuristic(start_position, goal_position) : std::numeric_limits<double>::infinity();
    ...
    while (!open_set.empty())
    {
        ...
        if (current_position == goal_position) {
            auto optimal_path = reconstruct_a_star_path(came_from, current_position);
            optimal_path.insert(optimal_path.begin(), start_position);
            graph_.set_astar_path(optimal_path);
            return;   
        }

        const auto current_vertex = graph_.get_vertex(current_position); // Use your method to get the vertex
        ...
        for (const auto& edge : current_vertex->get_edges())
        {
            ...
            if (edge_weight == -1) continue; // Skip untraversable edges
            ...
            if (tentative_g_score < g_score[neighbor_position])
            {
                ...
                f_score[neighbor_position] = g_score[neighbor_position] + graph_.get_heuristic(neighbor_position, goal_position);
    ...
    // No path found
    graph_.set_astar_path(std::vector<unsigned int>());
}
